...
@allure.step("Получение компании по {name}")
def test_get_list()-> str:
    name = "My company"
...
@allure.step("Создание нового сотрудника")
def test_add_new_employer()-> tuple[str,str,int]:
...
@allure.step("Получение конкретного сотрудника по {name}:{descr}:{id}")
...
@allure.title("Получение списка изменённых данных сотрудников")
@allure.description("Тест на изменение данных сотрудников")
@allure.feature("READ")
@allure.severity("blocker")


@allure.step("Изменение данных сотрудника")
def test_change_data()->tuple[int,str,str,int,bool]:
    db.create_company('Моя компания')
    max_id_c = db.get_max_id_comp()
